...
{
	return cedar_sys_phymalloc_map(size, align);
}


void  mem_pfree(void* p)
{
	cedar_sys_phyfree_map(p);
}
...
void  mem_flush_cache(u8* mem, u32 size)
{
	...
	cedarx_cache_op(mem, mem+size, CEDARX_DCACHE_CLEAN_FLUSH);
}


u32 mem_get_phy_addr(u32 virtual_addr)
{
	return (u32)cedarv_address_vir2phy((void*)virtual_addr);
}
