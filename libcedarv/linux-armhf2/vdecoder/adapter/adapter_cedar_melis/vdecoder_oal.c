        ...
        case VDRV_INTERNAL_CMD_SET_VBSINF:
        {
            ...
            switch(stream_info_oal->codec_type)
            {
                ...
                case VIDEO_DIVX1:
                {
                    ...
                    if (stream_info_oal->video_bs_src == CEDARLIB_FILE_FMT_MKV)
                        stream_info.container_format = CEDARV_CONTAINER_FORMAT_MKV;
                    
                    if (stream_info_oal->video_bs_src == CEDARLIB_FILE_FMT_MOV)
                        stream_info.container_format = CEDARV_CONTAINER_FORMAT_MOV;
                    
                    if (stream_info_oal->video_bs_src == CEDARLIB_FILE_FMT_FLV)
                        stream_info.container_format = CEDARV_CONTAINER_FORMAT_FLV;
                    
                     if (stream_info_oal->video_bs_src == CEDARLIB_FILE_FMT_AVI)
                        stream_info.container_format = CEDARV_CONTAINER_FORMAT_AVI;
                    ...
                    if(stream_info_oal->codec_type == VIDEO_XVID)
                    	...
                    	stream_info.sub_format = CEDARV_MPEG4_SUB_FORMAT_DIVX1;
                    else if(stream_info_oal->codec_type == VIDEO_DIVX2)
                    	stream_info.sub_format = CEDARV_MPEG4_SUB_FORMAT_DIVX2;
