...
static s32 fbm_alloc_frame_buffer(vpicture_t* picture)
{
	...
	memset(picture->y, 0, picture->size_y);
 	if(picture->size_u != 0)
 	{
 	...
 	memset(picture->u, 0, picture->size_u);
 	if(picture->size_v != 0)
 	{
 	...
 	memset(picture->v, 0, picture->size_v);
 	...
 	if(picture->size_y2){
 		...
 		memset(picture->y2, 0, picture->size_y2);
 	}
 	if(picture->size_u2){
 		...
 		memset(picture->u2, 0, picture->size_u2);
 	}
 	if(picture->size_v2){
 		...
 		memset(picture->v2, 0, picture->size_v2);
 	}
                ...
                pixel_format_e format)
{
    ...
    for (i=0; i<(s32)max_frame_num; i++)
    {
    	...
    	fbm->frames[i].picture.size_y2	  = 0;
    	fbm->frames[i].picture.size_u2	  = 0;
    	fbm->frames[i].picture.size_v2	  = 0;
    	fbm->frames[i].picture.size_alpha2 = 0;
    	if(g_vdecoder != NULL && g_vdecoder->status == CEDARV_STATUS_PREVIEW)
    	{
    		...
    		else
    		{
    			...
    			fbm->frames[i].picture.y2	 = NULL;
    			fbm->frames[i].picture.u2	 = NULL;
    			fbm->frames[i].picture.v2	 = NULL;
    			fbm->frames[i].picture.alpha = NULL;
    		}
    	}
    	else
    	{
                ...
                pixel_format_e format)
{
    ...
    for (i=0; i<(s32)max_frame_num; i++)
    {
    	...
    	fbm->frames[i].picture.output_3d_mode	= out_3d_mode;
    	...
    	if(g_vdecoder != NULL && g_vdecoder->status == CEDARV_STATUS_PREVIEW)
    	{
    		...
    		else
    		{
    			...
    			fbm->frames[i].picture.y2 	 = fbm->frames[0].picture.y2;
    			fbm->frames[i].picture.u2 	 = fbm->frames[0].picture.u2;
    			fbm->frames[i].picture.v2 	 = fbm->frames[0].picture.v2;
    			fbm->frames[i].picture.alpha2 = fbm->frames[0].picture.alpha2;
